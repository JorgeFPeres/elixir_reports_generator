defmodule ReportGenerator do
  ...
  def build_from_many(filesnames) when not is_list(filesnames) do
    {:error, "Please provide a list of strings"}
  end

  def build_from_many(filesnames) do
    result = filesnames
  ...
  def fetcher_higher_cost(report, option) when option in @options do
    {:ok, Enum.max_by(report[option], fn {_key, value} -> value end)}
  end

  def fetcher_higher_cost(_report, _option), do: {:error, "Invalid option!"}
